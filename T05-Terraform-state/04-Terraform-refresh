...
Scenario 1:
===========
1) Do changes to any resource which was actually created by terraform configuration to add new tags from AWS console.
2) Now execute #terraform refresh to capture the real-world changes to update the "terraform.tfstate" file.
3) Lets run #terraform plan and see the if those are reflected to make a decision about update in-place. But it will not update in terraform.tfstate file.
...
Note: what happens, when we do not run #terraform refresh and apply it directly.
